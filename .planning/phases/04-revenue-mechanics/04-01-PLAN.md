...
must_haves:
  truths:
    ...
    - "Prisma schema has RevenueEvent and RevenueClaim models with correct relations"
  artifacts:
    ...
    - path: "packages/utils/src/revenue-pda.ts"
      ...
      exports: ["getRevenueConfigPDA", "getRevenueEventPDA", "getRevenueClaimPDA"]
    - path: "apps/web/prisma/schema.prisma"
      provides: "RevenueEvent and RevenueClaim Prisma models"
      contains: "model RevenueEvent"
  key_links:
...
- RevenueEvent struct:
  - `event_index: u32`, `bump: u8`, `token: RevenueToken`, `total_amount: u64`
...
**Update state/mod.rs**: Add `pub mod revenue_config; pub mod revenue_event; pub mod revenue_claim;` and corresponding `pub use` lines.
...
- `BurnAlreadyExecuted` -- "Burn already executed for this event"
  ...
  <done>Three revenue state files compile, RevenueError enum has 11 error codes, state/mod.rs exports all new types</done>
</task>
...
BigInt columns follow the convention from governance models (e.g., Vote.weight, Idea.yesWeight). The `@@unique([revenueEventId, claimantWallet])` prevents double-claim at the database level mirroring the on-chain PDA uniqueness.
  </action>
  ...
  <done>Revenue types exported from @gsd/types, PDA helpers exported from @gsd/utils, Prisma schema validates with RevenueEvent and RevenueClaim models</done>
</task>
...
- Prisma schema validates with new RevenueEvent and RevenueClaim models
